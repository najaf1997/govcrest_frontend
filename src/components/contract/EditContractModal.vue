<template>
  ...
  >
    <template #modal-title>
      <h2 class="m-0 text-primary">Edit Contract</h2>
    </template>
    <validation-observer ref="editContractFormValidation">
...
export default {
  ...
  methods: {
    ...
    populateFields() {
      ...
      this.manufacturerPhone = this.contract.manufacturer_phone || "";
      this.manufacturerLink = this.contract.manufacturer_link || "";
      ...
      if (this.contract.poc) {
        // POC is already an array from backend serializer
        this.poc = Array.isArray(this.contract.poc)
          ? this.contract.poc
          : [this.contract.poc];
      }
    },
    ...
    async submit() {
      try {
        ...
        if (res.status === 200 || res.status === 201) {
          ...
          this.$nextTick(() => {
            this.$bvModal.hide("edit-contract-modal");
          });
